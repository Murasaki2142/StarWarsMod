...
import net.mcreator.starwarspathofthejedi.block.ConsoleBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class ConsoleTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public ConsoleTileEntity(BlockPos pos, BlockState state) {
		super(StarWarsPathOfTheJediModBlockEntities.CONSOLE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(ConsoleBlock.ANIMATION));
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<ConsoleTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<ConsoleTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
